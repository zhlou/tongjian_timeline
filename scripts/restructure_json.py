...
Extracts ganzhi (天干地支) and Western year from the parenthetical in
``era_year`` and strips the parenthetical.  Corrections for vol_name / vol_time_cycle
are now baked into raw_json_converted/ via fix_raw_converted.py.
"""
...
def convert_file(filepath, out_dir):
    ...
    grouped["volume_time_cycle"] = clean_volume_time_cycle(
        grouped.get("volume_time_cycle", ""), file_index)
